...
class Entry {
    ...
    static async fromGitHub(owner: string, repo: string, branch: string, path: string, locale: string, accessToken: string | undefined): Promise<Entry> {
        ...
        if (match) {
            ...
            if (locale !== "en-us") {
                return new Entry(metaData.title, metaData.slug, metaData.l10n?.sourceCommit ?? "no source commit yet", text.replace(match[0], "").trim());
            }
            ...
            if (response.status === 200 && response.data.length > 0) {
                return new Entry(metaData.title, metaData.slug, response.data[0].sha, text.replace(match[0], "").trim());
            } else {
